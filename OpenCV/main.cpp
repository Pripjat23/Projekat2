...
int suitMatcher(Mat rnk)                    //Suit der Karte matchen
{
	...
	// Sačuvaj ulaznu rank sliku (samo jednom)
static bool savedInput = false;
if (!savedInput) {
    stbi_write_png("current_rank.png", width, height, 1, rankImg.data(), width);
    system("start current_rank.png"); // Windows
    // system("xdg-open current_rank.png"); // Linux
    // system("open current_rank.png");     // macOS
    savedInput = true;
}

// Sačuvaj trenutni template
std::string outName = "template_compare_" + std::to_string(rank) + ".png";
stbi_write_png(outName.c_str(), tplW, tplH, 1, tplBinary.data(), tplW);

// Prikaži šablon
std::string cmd = "start " + outName; // Windows
// std::string cmd = "xdg-open " + outName; // Linux
// std::string cmd = "open " + outName; // macOS
system(cmd.c_str());
